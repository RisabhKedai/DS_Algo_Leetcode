/*
* @Author: lenovo
* @Date:   2024-03-24 21:50:52
* @Last Modified by:   lenovo
* @Last Modified time: 2024-03-24 21:52:28
*/
    ...
    for(int i = 0; i < nums.size(); ++i){               // iterate over each item in nums
        if(idToFreqMap[nums[i]]){                       // When freq is non-zero then remove its occurance from the freq to id map 
            ...
            if(freqToIdCountMap[idToFreqMap[nums[i]]] == 0) 
            	freqToIdCountMap.erase(idToFreqMap[nums[i]]);      // when freq to id map have current freq occuarance as zero then remove it from the map
        }
